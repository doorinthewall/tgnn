...
import sys
from tgnn.scripts.Models.BGNN_extended import BGNN_extended
...
class RunModel:
    ...
    def run_one_model(self, config_fn, model_name):
        ...
        for ps in grid:
            ...
            for _ in range(self.repeat_exp):
                ...
                if model_name in ['gnn', 'resgnn', 'bgnn', 'bgnn_extended']:
            ...
            if self.save_model_outputs:
                predictions = np.expand_dims(model.gbdt_model.predict(self.X), axis=1)
                node_feats = torch.FloatTensor(predictions).to(model.device)
                model.model.eval()
                graph = model.networkx_to_torch(self.networkx_graph)
                graph = graph.to(model.device)
                model_in = (graph, node_feats)
                logits = model.model.l1(*model_in).flatten(1).detach().cpu().numpy()
                self.outputs = {'gnn':logits, 'tab':predictions}
    ...
    def define_model(self, model_name, ps):
        ...
        elif model_name == 'bgnn_extended':
            return BGNN_extended(task=self.task, **ps)
    ...
    def aggregate_results(self):
        algos = ['catboost', 'lightgbm', 'mlp', 'gnn', 'resgnn', 'bgnn', 'bgnn_extended']
    ...
    def run(self, dataset: str, *args,
        ...
        self.save_model_outputs = save_model_outputs
        ...
        for ix, seed in enumerate(self.masks):
            ...
            for arg in args:
                if arg == 'all':
                    ...
                    self.run_one_model(config_fn=config_dir / 'bgnn.yaml', model_name="bgnn")
                elif arg == 'bgnn_extended':
                    self.run_one_model(config_fn=config_dir / 'bgnn_extended.yaml', model_name="bgnn_extended")
                    if self.save_model_outputs:
                        path = os.path.join(self.save_folder, seed)
                        with open(os.path.join(path,'output.pkl'), 'wb') as fp:
                            pk.dump(self.outputs, fp)
